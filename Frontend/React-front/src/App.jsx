...
export default function App() {
  ...
  const renderContent = () => {
    switch(currentTab) {
      ...
      case 'Fuel & Expenses': return <Expenses />;
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-surface)' }}>
      ...
      <div style={{ flex: 1, padding: '40px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '32px' }}>
        {/* Top Context Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#ffffff', padding: '16px 24px', borderRadius: '12px', border: '1px solid var(--border-muted)' }}>
          <span style={{ fontWeight: '700', fontSize: '0.95rem' }}>Active Workspace Context Engine</span>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '0.85rem', fontWeight: '600' }}>
            <span style={{ color: 'var(--text-muted)' }}>Current Layer:</span>
            <span style={{ background: '#f1f2f4', padding: '4px 10px', borderRadius: '4px' }}>{currentTab.toUpperCase()}</span>
          </div>
        </div>
